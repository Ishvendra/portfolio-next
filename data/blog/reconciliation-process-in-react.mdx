...
That's essentially what the Virtual DOM is. It's a **lightweight, in-memory representation of the actual DOM**. It's just a plain object (or a tree of objects) that mirrors the structure and properties of the browser's DOM.

When you write a React component, you're not directly creating DOM elements. You're describing what your UI _should_ look like using JSX. This JSX then gets compiled into React "elements" ( objects), which form the Virtual DOM tree.
...
// React internally represents this as a tree of  objects:
// {
//   type: 'div',
//   props: { children: [
...
Passing new object or array literals directly as props will cause `React.memo`ized components to re-render because `{} !== {}` and `[] !== []` in (referential inequality).
